...
def main():
    ...
    parser.add_argument("--output_file",
                    default=None,
                    type=str,
                    help="The output file to save the plot (optional).")
    ...
    for i in range(len(proportion_all)):
        epsilon = 1e-10
        proportion_all[i] /= abs(proportion_all[i][1:, :].max()) + epsilon
        proportion_all[i] = np.nan_to_num(proportion_all[i])
    ...
    if args.output_file:
        plt.savefig(args.output_file, format="png", dpi=300)
